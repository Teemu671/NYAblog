...
function createPostCard(post, user, image) {
  ...
  const date = escapeHtml(formatDate(post.updated_at));
  ...
  wrapper.innerHTML = `
    <a class="card" href="/blogPage?postId=${postId}">
      <img src="${ post.image_id ? 'https://cat0s.com/cdn/'+image.filename : 'https://cat0s.com/cdn/placeholder.png'}" class="card-img-top" alt="${tag}">
      <div class="card-body">
